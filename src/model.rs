...
type Assignment = HashMap<Var, VarState>;
type DecisionStack<'a> = Vec<(i32, Lit, Option<&'a Clause>)>;

fn decide<'a>(decision_stack: &mut DecisionStack<'a>,
...
fn get_assertion_lit(decision_stack: &DecisionStack,
                     clause: &Clause,
                     dl: &i32
) -> Option<Lit> {
...
pub fn solve(clauses: &mut Vec<Clause>) -> bool {
        ...
        loop {
            match &mut solver_state {
                SolverState::Resolving(conflict) => {
                    ...
                    if let Some(assertion_lit) = get_assertion_lit(&decision_stack,
                                                                   conflict,
                                                                   &dl) {
